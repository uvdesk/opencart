...
class ControllerUvdeskUvdesk extends Controller {
	...
	public function index() {
		...
		$data['tab_active'] = $tab;
		$data['tab_url'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . $url . '&tab=', true);
		...
		$data['label_url'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . '&label=', true);
		$data['custom_label_active'] = $custom_label;
		$data['custom_label_url'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . '&custom_label=', true);
		$data['search'] = $search;
		$data['search_url'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . $url . '&search=', true);

		$data['view'] = $this->url->link('uvdesk/uvdesk/view', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('uvdesk/uvdesk/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);
		...
		if (isset($tickets->error) || !$tickets) {
			...
			$this->response->setOutput($this->load->view('uvdesk/error_page', $data));
			return;
		}
		...
		foreach ($results as $result) {
			$data['tickets'][] = array(
				...
				'view'       => $this->url->link('uvdesk/uvdesk/view', 'id=' . $result->incrementId . '&user_token=' . $this->session->data['user_token'], true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url, true);
		$data['sort_status'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . '&sort=status' . $url, true);
		...
		$pagination->url = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
	...
	public function view() {
		...
		if (isset($this->request->get['id']) && $this->request->get['id']) {
			...
			$data['predefined_labels'] = array();

			foreach ($ticket->labels->predefind as $key => $value) {
				$data['predefined_labels'][$key] = $value;
			}
		...
		$data['label_active'] = $label;
		$data['label_url'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . '&label=', true);
		$data['attachment_url'] = $this->url->link('uvdesk/uvdesk/download', 'user_token=' . $this->session->data['user_token'] . '&attachment=', true);
		$data['custom_label_active'] = $custom_label;
		$data['custom_label_url'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . '&custom_label=', true);
		$data['user_token'] = $this->session->data['user_token'];

		$data['add_reply'] = $this->url->link('uvdesk/uvdesk/addReply', 'user_token=' . $this->session->data['user_token'], true);
		...
		$this->response->setOutput($this->load->view('uvdesk/ticket_view', $data));
	}
	...
	public function addReply()	{
		...
		if (isset($this->request->post['reply']) && $this->request->post['reply']) {
			...
			$this->response->redirect($this->url->link('uvdesk/uvdesk/view', 'id=' . $this->request->post['id'] . '&user_token=' . $this->session->data['user_token'], true));
		}
	...
	public function filteredTickets() {
		...
		if (!$isAjax) {
			$this->response->redirect($this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'], true));
		}
		...
		$data['tab_active'] = $tab;
		$data['tab_url'] = $this->url->link('uvdesk/uvdesk', 'user_token=' . $this->session->data['user_token'] . $url . '&tab=', true);
		$data['delete'] = $this->url->link('uvdesk/uvdesk/delete', 'user_token=' . $this->session->data['user_token'], true);

		$pagination = new Pagination();
		$pagination->total = $ticket_total;
		$pagination->page = $page;
		$pagination->limit = $items_on_page;
		$pagination->url = $this->url->link('uvdesk/uvdesk/filteredTickets', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($ticket_total) ? (($page - 1) * $items_on_page) + 1 : 0, ((($page - 1) * $items_on_page) > ($ticket_total - $items_on_page)) ? $ticket_total : ((($page - 1) * $items_on_page) + $items_on_page), $ticket_total, ceil($ticket_total / $items_on_page));

		return $this->response->setOutput($this->load->view('uvdesk/list_view', $data));
	}
	...
	public function download() {
		if (isset($this->request->get['attachment']) && $this->request->get['attachment']) {
			...
			$access_token = $this->config->get('module_uvdesk_access_token');
